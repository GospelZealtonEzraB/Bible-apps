 ...
 *
 * This is a COMPONENT, not a screen, so the Chat tab can render the thread in
 * place — the tab bar stays put while you talk, the way a messaging app works.
 * (It is also mounted as a standalone route when you open a thread anchored to
 * a verse.) Rendering it inline also avoids redirecting a tab to a route that
 * lives outside the tab navigator, which tears the navigator down mid-focus.
 */
export function Conversation({
  ...
  code: string;
  /** When set, the thread is anchored to this reference. */
  context?: string | null;
  /** Standalone route → show a back chevron. In the tab there is nowhere back to. */
  showBack?: boolean;
}) {
      ...
      >
        ...
        <View
          style={{
            ...
            paddingBottom: showBack ? Math.max(insets.bottom, spacing.sm) : spacing.sm,
            borderTopWidth: 1,
