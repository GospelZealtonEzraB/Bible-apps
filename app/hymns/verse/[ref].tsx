...
import { hymnsForRef, matchHymnByTitle, type HymnRefMatch } from '@/data/hymns';
...
/**
 * "Songs from this verse" — the bidirectional flip of the hymnal. Two tiers:
 * (1) GROUNDED: bundled public-domain hymns whose scriptureRefs actually match
 *     this verse — real, in-app, tappable.
...
export default function SongsForVerseScreen() {
  ...
  const grounded: HymnRefMatch[] = reference ? hymnsForRef(reference) : [];
    ...
    <Screen>
      ...
      {grounded.length > 0 ? (
        <View style={{ gap: spacing.sm }}>
          <SectionTitle>In our hymnal</SectionTitle>
          {grounded.map(({ hymn, precision }) => (
            <Card key={hymn.id} onPress={() => router.push(`/hymns/${hymn.id}`)} style={{ flexDirection: 'row', alignItems: 'center', gap: spacing.md }}>
      ...
      <View style={{ gap: spacing.sm }}>
        ...
        {ai && ai.length > 0 ? (
          <>
            ...
            {ai.map((song, i) => {
              const inApp = matchHymnByTitle(song.title);
              ...
              return (
                <Card key={`${song.title}-${i}`} style={{ gap: spacing.sm }}>
                  ...
                  <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: spacing.sm }}>
                    {inApp ? (
                      <LinkChip icon="open-outline" label="Open in hymnal" primary onPress={() => router.push(`/hymns/${inApp.id}`)} />
                    ) : null}
